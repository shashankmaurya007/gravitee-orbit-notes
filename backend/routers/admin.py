...
async def get_push_schedule():
    ...
    return {**cfg, **next_push_run_info(), "day_options": _DAY_OPTIONS, "tz_options": _TIMEZONE_OPTIONS, "activity_level_options": _ACTIVITY_LEVELS}


_ACTIVITY_LEVELS = ["significant", "low", "none"]
...
async def set_push_schedule(body: dict):
    ...
    if "minute"          in body: cfg["minute"]           = int(body["minute"])
    if "timezone"        in body: cfg["timezone"]         = str(body["timezone"])
    if "activity_levels" in body:
        levels = [l for l in body["activity_levels"] if l in _ACTIVITY_LEVELS]
        cfg["activity_levels"] = levels
    save_push_config(cfg)
    apply_push_schedule(cfg)
    return {**cfg, **next_push_run_info(), "activity_level_options": _ACTIVITY_LEVELS}
